...
public class MousePicker {
	...
	public static void setUpMousePicker(World wor, Camera cam) {
		world = wor;
		camera = cam;
	...
	public static Vector3f getPointOnYPlane(float height) {
		if (intersectionInRange(0, RAY_RANGE, currentRay)) {
			float distance = (height - camera.getY()) / (currentRay.y);
	...
	private static boolean isUnderGround(Vector3f testPoint) {
		...
		if (terrain != null) {
			...
			/*height = terrain.calcHeight(testPoint.getX() - terrain.getX(), testPoint.getZ()
					- terrain.getZ());*/
		}
	...
	private static Vector3f getPointOnRay(Vector3f ray, float distance) {
		Vector3f start = new Vector3f(camera.getX(), camera.getY(), camera.getZ());
